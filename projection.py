...
def read_file(fname: str, tree: str, event_index: int):
    with up.open(fname) as f:
        ...
        Hit_Energy = 0.001 * tree['Hit_Energy'].array(library='np')[event_index]
...
def plot_axis(fname: str, tree: str, event_index: int, title: str):
    ...
    ax_z.set_ylabel("Energy [GeV]", size='large')

    # Projection on x axis
    n_x, bins_x, patches_x = ax_x.hist(x, bins=nCellX, range=[-0.5 * WidthX, 0.5 * WidthX], weights=energy_x)
    height = n_x / n_x.max()
    ...
    ax_x.set_ylabel("Energy [GeV]", size='large')

    # Projection on y axis
    n_y, bins_y, patches_y = ax_y.hist(y, bins=nCellY, range=[-0.5 * WidthY, 0.5 * WidthY], weights=energy_y)
    height = n_y / n_y.max()
    ...
    ax_y.set_ylabel("Energy [GeV]", size='large')

    fig.suptitle(title, size='xx-large')
    plt.tight_layout()


def plot_plane(fname: str, tree: str, event_index: int, title: str):
    Hit_X, Hit_Y, Hit_Z, Hit_Energy = read_file(fname, tree, event_index)
    ...
    plt.gca().set_box_aspect((ratioX_xz, ratioY, ratioZ))
    ax_yz = fig.add_subplot(spec[1, 1], projection='3d')
    plt.gca().set_box_aspect((ratioX_yz, ratioY, ratioZ))

    # Projection on xOy plane
    hist_xy, xedges_xy, yedges_xy = np.histogram2d(x_xy, y_xy, bins=[nCellX, nCellY], range=[[-0.5 * WidthX, 0.5 * WidthX], [-0.5 * WidthY, 0.5 * WidthY]],  weights=energy_xy)
    ...
    ax_xy.set_zlabel("Energy [GeV]", size='x-large')
    ax_xy.view_init(elev=15, azim=-60, roll=0)

    # Projection on xOz plane
    hist_xz, xedges_xz, yedges_xz = np.histogram2d(x_xz, z_xz, bins=[nCellX, nLayer], range=[[-0.5 * WidthX, 0.5 * WidthX], [0, WidthZ]],  weights=energy_xz)
    ...
    ax_xz.set_zlabel("Energy [GeV]", size='large')
    ax_xz.view_init(elev=10, azim=-40, roll=0)

    # Projection on yOz plane
    hist_yz, xedges_yz, yedges_yz = np.histogram2d(y_yz, z_yz, bins=[nCellY, nLayer], range=[[-0.5 * WidthY, 0.5 * WidthY], [0, WidthZ]],  weights=energy_yz)
    ...
    ax_yz.set_zlabel("Energy [GeV]", size='large')
...
if __name__ == '__main__':
    ...
    plot_axis(filename, tree, event_index, title)
    ...
    plot_plane(filename, tree, event_index, title)
