...
def plot(fname: str, tree: str, event_index: int, title: str):
    ...
    hist, xedges, yedges = np.histogram2d(x_xy, y_xy, bins=[nCellX, nCellY], range=[[-0.5 * nCellX * CellWidthX, 0.5 * nCellX * CellWidthX], [-0.5 * nCellY * CellWidthY, 0.5 * nCellY * CellWidthY]],  weights=energy_xy)
    ...
    ax.set_xlim(-0.5 * nCellX * CellWidthX, 0.5 * nCellX * CellWidthX)
    ax.set_ylim(-0.5 * nCellY * CellWidthY, 0.5 * nCellY * CellWidthY)

    ax.set_xlabel("X [cm]", size='x-large')
    ax.set_ylabel("Y [cm]", size='x-large')
    ax.set_zlabel("Energy [GeV]", size='x-large')
