...
//    EnableImplicitMT();
    ...
    .Define("COG_X_mean", [] (const vector<Double_t>& Hit_X, const vector<Double_t>& Hit_Energy, const Double_t& Edep, const Int_t& nhits)->Double_t
    {
        ...
        if (Edep == 0)
            return 0.5 * nCellsX * CellWidthX + 10;
        for (Int_t i = 0; i < nhits; ++i)
    ...
    .Define("COG_Y_mean", [] (const vector<Double_t>& Hit_Y, const vector<Double_t>& Hit_Energy, const Double_t& Edep, const Int_t& nhits)->Double_t
    {
        ...
        if (Edep == 0)
            return 0.5 * nCellsY * CellWidthY + 10;
        for (Int_t i = 0; i < nhits; ++i)
    ...
    {
        TH1D* h = new TH1D("h", "", 120, -30, 30);
        for (Double_t i : Hit_X)
            h->Fill(i - COG_X_mean);
        Double_t xwidth = h->GetRMS();
        delete h;
        return xwidth;
    }, {"Hit_X", "COG_X_mean"})
    ...
    {
        TH1D* h = new TH1D("h", "", nLayer + 1, 0, (nLayer + 1) * Thick);
        for (Double_t i : Hit_Z)
            h->Fill(i);
        Double_t zdepth = h->GetRMS();
        delete h;
        return zdepth;
    }, {"Hit_Z"})
    ...
    .Define("Ecell", [] (const vector<Int_t>& CellID, const vector<Double_t>& Hit_Energy, const Int_t& nhits)->vector<vector<Double_t>>
    {
        ...
        for (auto i : Ecell_map)
        {
            Ecell.at(0).emplace_back(i.first);
            Ecell.at(1).emplace_back(i.second);
        }
        Ecell_map.clear();
        return Ecell;
    ...
    .Define("Emax_sec_dist", [] (const vector<Double_t>& Ecell_max_id, const vector<Double_t>& Ecell_second_id)->Double_t
    {
        ...
        Double_t x_max = (Int_t) round(Ecell_max_id.at(0)) % 100 * CellWidthX;
        Double_t y_max = ((Int_t) round(Ecell_max_id.at(0)) % 10000) / 100 * CellWidthY;
        Double_t z_max = (Int_t) round(Ecell_max_id.at(0)) / 10000 * Thick;
        Double_t x_second = (Int_t) round(Ecell_second_id.at(0)) % 100 * CellWidthX;
        Double_t y_second = ((Int_t) round(Ecell_second_id.at(0)) % 10000) / 100 * CellWidthY;
        Double_t z_second = (Int_t) round(Ecell_second_id.at(0)) / 10000 * Thick;
    ...
    .Define("Ecell_max_9", [] (const vector<vector<Double_t>>& Ecell, const vector<Double_t>& Ecell_max_id)->Double_t
    {
        ...
        Int_t x = (Int_t) Ecell_max_id.at(0) % 100 - 1;
        Int_t y = ((Int_t) Ecell_max_id.at(0) % 10000) / 100 - 1;
        Int_t z = (Int_t) Ecell_max_id.at(0) / 10000 - 1;
        for (Int_t ix = x - 1; ix <= x + 1; ++ix)
        {
            ...
            for (Int_t iy = y - 1; iy <= y + 1; ++iy)
            {
                if (iy < 0 || iy >= nCellsY)
                    continue;
                Int_t tmp = (z + 1) * 10000 + (iy + 1) * 100 + ix + 1;
    ...
    .Define("layer_rms", [] (const vector<Double_t>& Hit_X, const vector<Double_t>& Hit_Y, const vector<Int_t>& layer, const vector<Double_t>& Hit_Energy, const Int_t& nhits, const Double_t& COG_X_mean, const Double_t& COG_Y_mean)->vector<Double_t>
    {
        ...
        for (Int_t i = 0; i < nLayer; ++i)
            h.emplace_back(new TH2D("h" + TString(to_string(i)), "", 120, -30, 30, 120, -30, 30));
        for (Int_t i = 0; i < nhits; ++i)
        {
            Int_t ilayer = layer.at(i);
            h.at(ilayer)->Fill(Hit_X.at(i) - COG_X_mean, Hit_Y.at(i) - COG_Y_mean, Hit_Energy.at(i));
        }
        for (Int_t i = 0; i < h.size(); ++i)
        {
            if (h.at(i)->GetEntries() < 4)
                layer_rms.at(i) = 0.0;
            else
                layer_rms.at(i) = h.at(i)->GetRMS();
            delete h.at(i);
        }
        vector<TH2D*>().swap(h);
        return layer_rms;
    ...
    .Define("shower_start", [] (const vector<Int_t>& hits_on_layer)->Int_t
    {
        ...
        const Int_t hit_threshold = 4;
        for (Int_t i = 0; i < nLayer - 3; ++i)
            if (hits_on_layer.at(i) >= hit_threshold && hits_on_layer.at(i + 1) >= hit_threshold && hits_on_layer.at(i + 2) >= hit_threshold && hits_on_layer.at(i + 3) >= hit_threshold)
            {
                shower_start = i + 1;
                break;
            }
    ...
    .Define("shower_end", [] (const vector<Int_t>& hits_on_layer, const Int_t& shower_start)->Int_t
    {
        ...
        if (shower_start == -1)
            return -1;
        for (Int_t i = shower_start - 1; i < nLayer - 2; ++i)
    ...
    .Define("layer_xwidth", [] (const vector<Double_t>& Hit_X, const vector<Int_t>& layer, const Int_t& nhits, const Double_t& COG_X_mean)->vector<Double_t>
    {
        ...
        for (Int_t l = 0; l < nLayer; ++l)
            h.emplace_back(new TH1D(TString("h") + TString(to_string(l)), "", 120, -30, 30));
        for (Int_t i = 0; i < nhits; ++i)
        {
            Int_t ilayer = layer.at(i);
    ...
    .Define("shower_density", [] (const vector<Int_t>& CellID, const Int_t& nhits)->Double_t
    {
        ...
        {
            Int_t x = CellID.at(j) % 100 - 1;
            Int_t y = (CellID.at(j) % 10000) / 100 - 1;
            Int_t z = CellID.at(j) / 10000 - 1;
            for (Int_t ix = x - 1; ix <= x + 1; ++ix)
            {
...
//        Int_t max_layer = maxPosition - layer_rms.begin();
    ...
    .Define("FD_2D", [] (const vector<Double_t>& Hit_X, const vector<Double_t>& Hit_Y, const vector<Double_t>& Hit_Z, const Int_t& nhits)->vector<Double_t>
    {
        ...
        vector<Int_t> nhits_resized(num);
        for (Int_t i = 0; i < num; ++i)
        {
            nhits_resized.at(i) = NewScale(Hit_X, Hit_Y, Hit_Z, scale.at(i), scale.at(i), 1);
            if (nhits == 0 || nhits_resized.at(i) <= 0)
            {
                fd_2d.at(i) = -1.0;
                continue;
            }
            fd_2d.at(i) = Log((Double_t) nhits / nhits_resized.at(i)) / Log((Double_t) scale.at(i));
        }
        return fd_2d;
    ...
    .Define("FD_3D", [] (const vector<Double_t>& Hit_X, const vector<Double_t>& Hit_Y, const vector<Double_t>& Hit_Z, const Int_t& nhits)->vector<Double_t>
    {
        ...
        for (Int_t i = 0; i < num; ++i)
        {
            nhits_resized.at(i) = NewScale(Hit_X, Hit_Y, Hit_Z, scale.at(i), scale.at(i), scale.at(i));
            if (nhits == 0 || nhits_resized.at(i) <= 0)
            {
                fd_3d.at(i) = -1.0;
                continue;
            }
            fd_3d.at(i) = Log((Double_t) nhits / nhits_resized.at(i)) / Log((Double_t) scale.at(i));
        }
        return fd_3d;
    }, {"Hit_X", "Hit_Y", "Hit_Z", "nhits"})
...
//    .Range(1)
    ...
    const vector<TString> deactivate = { "CellID", "Ecell", "Ecell_max_id", "Ecell_second_id", "FD_2D", "FD_3D", "Hit_Energy", "Hit_Time_first", "Hit_Time_maxE", "Hit_X", "Hit_Y", "Hit_Z", "hits_on_layer", "layer", "layer_energy", "layer_rms", "layer_xwidth", "layer_ywidth" };
